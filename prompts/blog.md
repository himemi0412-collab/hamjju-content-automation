...
- 본문 끝에는 실제 확인한 제조사·기관 공식 원문 링크를 일반 Markdown 링크로 기록하고 확인 기준일을 YYYY-MM-DD로 남긴다. 내부 검색 참조 ID나 cite 토큰은 최종 원고에 넣지 않는다. 출처 목록에는 페이지 제목, 실제 URL, 확인일과 뒷받침하는 주장만 적는다.
...
카드뉴스 기준:
- 정확히 5장, 정사각형 1080×1080: 질문 표지(cover) → 원리 흐름(flow) → 동일 기준 비교(comparison) → 행동 체크(checklist) → 선택 기준 요약(decision).
...
- 각 카드에는 layout, illustration, items, caption이 필수다. illustration은 주제와 맞는 bubbles(세제·거품), laundry(직물·이불), wifi(통신·벽), document(설명서·조건 확인) 중 하나다. 상상한 제품 외형·로고·확인되지 않은 숫자를 넣지 않는다.
