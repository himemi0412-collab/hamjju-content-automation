...
- 건조기 카드별로 필터 장착부 근접 확인, 물통·드럼의 안전한 확인(분리 부품 없이), 측면 여유와 주변 물건, 손으로 보이는 범위 점검, 서비스 문의 판단 장면이 서로 구별되어야 한다. 특히 비교 역할은 실제 두 조건이 같은 사진 안에서 비교 가능한 크기로 보여야 한다.
...
- 같은 제품을 반복하는 장면은 카드 역할별 크롭·각도·거리·행동·배경이 의미 있게 달라야 한다. 다섯 장이 같은 정면 제품 사진이나 광고용 제품컷처럼 보이면 기계적 반복과 스톡 광고 분위기로 AI 티 점수를 반영하고 실패시킨다.
- 건조기 글은 기사에서 명시하지 않은 외부 배기 호스·후면 덕트·분리된 환기 연결부를 보여주지 않는다. 대신 측면 여유 공간, 주변 먼지·잡동사니, 보이는 전면/측면 점검을 나타낸다.
