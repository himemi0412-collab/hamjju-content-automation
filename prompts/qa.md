...
- 실제 PNG의 글자 깨짐·잘림·겹침, 잘못된 도식/화살표, 카드 간 모순, 반복 글상자만 있는 구성, 확인되지 않은 수치/제품 형태/실사용 주장, 캡션·설명용 고지를 확인한다. 한 장이라도 읽을 수 없거나 의미가 틀리면 blocking_issues에 카드 번호와 이유를 남긴다.
- 카드마다 본문 이해를 돕는 도식·비교·체크리스트·사물 중 하나가 있으며 역할이 서로 다른지 확인한다. 특정 모델 사진이 없는 주제는 무브랜드 설명 도식을 허용한다.
