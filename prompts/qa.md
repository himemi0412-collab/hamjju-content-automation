중요 채널 매핑: 내부 키 `ppojjugi_shorts`와 Notion 표시 이름 `햄찌 창작 쇼츠`는 같은 채널이며, `japan_shorts`와 `일본 유튜브 쇼츠`도 같은 채널이다. 이 정상 매핑만으로 채널 충돌 또는 다른 채널 설정 혼입으로 판정하지 않는다. 실제 대본, 등장인물, 화면 구성, 언어가 다른 채널 규칙을 사용했을 때만 차단한다.
