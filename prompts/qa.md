...
- 실제 PNG에서 사진형 장면·생활 사물·화면·설명 도식이 체감 면적의 60~70%를 차지하고 한글 정보 영역이 30~40% 이하인지 확인한다. 큰 한글이 첫인상을 독점하거나, 제목 아래에 작은 아이콘만 붙어 있거나, 두 장 이상이 사실상 텍스트 카드라면 `EDITOR_FORMAT_PASS=false`다.
- headline은 공백 포함 18자, copy는 공백 포함 42자를 넘지 않아야 한다. 생성 장면 내부의 깨진 한글·영문·가짜 UI 문자는 차단하며, 모든 실제 문구는 코드 타이포그래피 레이어로만 합성되어야 한다.
...
- body_markdown 끝의 `공식 확인 출처`에 입력에서 제공된 공식 원문 URL과 확인일을 그대로 보존했는가. 같은 기관의 다른 주소로 임의 교체하지 않았는가.
- 모델명·제조사·설명서·고객지원 확인을 표현하는 카드에는 Wi-Fi/신호 아이콘 대신 appliance 또는 document 계열의 구체적인 사물 도식을 사용했는가.
