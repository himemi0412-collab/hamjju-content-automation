...
- 매일 09:45 KST에 GitHub Actions가 블로그 3건, 삐죽이 1건, 일본 레트로 1건의
  준비된 대기열 수량을 확인합니다.
- 필요한 수량이 이미 있으면 웹 검색과 AI 호출을 생략합니다.
- 부족할 때만 검색 의도, 대표·보조 키워드, GEO 핵심 답변, 후속 질문, 시기성,
  확인 출처와 확인일을 조사합니다.
- SEO·GEO 점수가 각각 70점 이상이고 필수 근거가 있는 항목만 Notion에 저장합니다.
- 새 Notion 속성을 강제로 만들지 않고 블로그의 기존 `대표 키워드`, `보조 키워드`,
  `독자 질문`, `출처 목록`과 쇼츠의 `작업 기록`, `출처·확인일`을 사용합니다.

갑자기 원하는 소재를 직접 추가할 때는 Notion에서 기존 제작 조건을 채운 뒤
블로그의 `키워드 출처`를 `직접 입력`으로 선택하거나, 쇼츠의 `다음 행동`에
`사용자 직접 추가`라고 적습니다. 이 표식이 있는 항목은 자동 수집 주제보다 먼저 처리됩니다.
