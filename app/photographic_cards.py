...
def _subject_lock(card: dict[str, Any], index: int | None = None) -> str:
    ...
    locks = (
        ...
        (('건조기',), 'Show a recognizable, intact front-loading tumble clothes dryer with a circular door and its lint filter at the door or lower opening. Keep the dryer itself visible when showing the filter, drum or ventilation space. Show only non-invasive inspection of visible areas: never depict disassembly, detached trays or tanks on the floor, washing components, wet parts or water droplets unless the card explicitly instructs that exact safe action. Never show an air purifier, dehumidifier or water tank appliance.'),
